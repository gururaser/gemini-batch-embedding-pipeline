...
def run_verify(settings: Settings | None = None) -> None:
    ...
    # 4. Cross-modal: text query via text_template should retrieve self in top-5
    cfg = DatasetConfig.from_yaml("dataset.yaml")
    if not cfg.text_template or not cfg.template_columns():
        results["Cross-modal sanity"] = "[yellow]SKIP[/yellow] (no text_template configured)"
    else:
        with get_conn(settings.state_db) as conn:
            sample_rows = conn.execute(
                """
                SELECT article_id, point_uuid, payload_json FROM records
        ...
        for row in sample_rows:
            ...
            try:
                query_text = cfg.text_template.format_map(payload)
            except KeyError:
                continue
            ...
            try:
                ...
                top_ids = {str(r.id) for r in search_results}
                if str(row["point_uuid"]) in top_ids:
                    cross_modal_ok += 1
            except Exception as e:
                console.log(f"[yellow]Cross-modal query failed: {e}[/yellow]")

        results["Cross-modal sanity"] = (
            f"[green]PASS[/green] ({cross_modal_ok}/{len(sample_rows)} self in top-5)"
            if cross_modal_ok >= len(sample_rows) // 2
            else f"[yellow]WARN[/yellow] ({cross_modal_ok}/{len(sample_rows)} self in top-5)"
        )
